    ...
    return (
        <div className='h-fit p-[0.75rem_0.75rem_1.50rem_0.75rem] border-2 border-solid border-text grid grid-cols-[repeat(3,1fr)] gap-1'>
            <Image
                ...
                height={200}
            />
            <h3 className='text-center flex justify-center items-center col-[2/span_2]'>{item.name}</h3>
            ...
            <div className='flex justify-row items-center col-[1] '>
                <button className='w-6 h-6 flex justify-center items-center text-center border-[1pt] border-solid border-text text-2xl m-[0.25rem_0.1rem] ' onClick={() => changeQty(qty - 1)}>
                    {qty == 1 ? (
                        <Image
                ...
                <p className='w-6 h-6 flex justify-center items-center text-center p-4'>{qty}</p>
                <button className='w-6 h-6 flex justify-center items-center text-center border-[1px] border-solid border-text text-2xl m-[0.25rem_0.1rem] ' onClick={() => changeQty(qty + 1)}>+</button>
            </div>
...
export function CustomerOrderSidebar({
    ...
    return (
        <div className='border-l-2 border-solid border-l-text h-full w-80'>
            <div className='h-4/5 overflow-y-scroll overflow-x-hidden'>{children}</div>
            <Link href={checkoutPage}>
                <button
                    className={'w-full h-1/5 bg-secondary border-2 border-solid border-text text-4xl' + (currentOrder.length === 0 ? ' hidden' : '')}
