...
export async function getIngredientsByMenuItemId(item_id: number, tsql = psql): Promise<Ingredient[]> {
    return transact<Ingredient[], postgres.Error, any>(tsql, new Error("SQL Error in getIngredientsByMenuItemId", undefined, {item_id: item_id}), async (isql, _) => {
        const result = await isql
        `SELECT inventory_id, amount FROM ingredients WHERE item_id = ${item_id}`;
...
export async function addMenuItem(menuItem: MenuItem, tsql = psql): Promise<boolean> {
    return transact<boolean, postgres.Error, any>(tsql, new Error("SQL Error in addMenuItem", undefined, {menuItem: menuItem}), async (isql, _) => {
        ...
        const addedMenuItem = new MenuItem(addedId,menuItem.name,menuItem.type,menuItem.price,menuItem.netPrice,menuItem.popularity,menuItem.ingredients,menuItem.seasonal); //creates new menu item

        if(menuItem.seasonal != null && !await addSeasonalItem(addedMenuItem, tsql)){ //if it doesnt meet conditions
            return false;
        }
...
export async function addIngredients(menuItem: MenuItem, tsql = psql): Promise<boolean> {
    return transact<boolean, postgres.Error, any>(tsql, new Error("SQL Error in addIngredients", undefined, {menuItem: menuItem}), async (isql, _) => {
        ...
        for(const ingredient of menuItem.ingredients){
            ...
            const inventoryId = await findInventoryIdByName(ingredient.inventoryItem.name, tsql); //if no inventory id is found
            if(inventoryId == -1){
                return false;
            }

            const added = await addIngredient(menuItem.id, inventoryId, ingredient.amount, tsql); //if add does not work
            if(!added){
                return false
            }
...
export async function addIngredient(item_id: number, inventory_id: number, amount: number, tsql = psql): Promise<boolean> {
    return transact<boolean, postgres.Error, any>(tsql, new Error("SQL Error in addIngredient", undefined, {item_id: item_id, inventory_id: inventory_id, amount: amount}), async (isql, _) => {
        ...
        if(result.length > 0){ //if at least a parameter exists
            return true;
        }
...
export async function updateMenuItem(menuItem: MenuItem, tsql = psql): Promise<boolean> {
    return transact<boolean, postgres.Error, any>(tsql, new Error("SQL Error in updateMenuItem", undefined, {menuItem: MenuItem}), async (isql, _) => {
        ...
        if(result.length == 0){ //if no param exists
            return false;
        }

        const updatedMenuItem = new MenuItem(itemId,menuItem.name,menuItem.type,menuItem.price,menuItem.netPrice,menuItem.popularity,menuItem.ingredients,menuItem.seasonal); //update content
    
        const seasonalItemUpdated = await updateSeasonalItem(updatedMenuItem, tsql);
        const ingredientsUpdated = await updateIngredients(updatedMenuItem, tsql);
...
export async function updateIngredients(menuItem: MenuItem, tsql = psql): Promise<boolean> {
    return transact<boolean, postgres.Error, any>(tsql, new Error("SQL Error in updateIngredients", undefined, {name: name}), async (isql, _) => {
        ...
        const currentIngredients = await getIngredientsByMenuItemId(menuItem.id, tsql); //gets all ingredients with menu item id

        for (const currentIngredient of currentIngredients){ //iterate through all ingredients

            let found = false;

            for (const newIngredient of menuItem.ingredients){

                if (newIngredient.inventoryItem.name == currentIngredient.inventoryItem.name){ //if inventory name matches 
                    found = true;

                    if(newIngredient.amount == 0){ //if there is no amount for a certain ingredient
                        if (!(await deleteIngredient(menuItem.id, await findInventoryIdByName(newIngredient.inventoryItem.name,tsql),tsql))) {
                            return false;
                        }                    
                    } else if (newIngredient.amount != currentIngredient.amount) { //if new amount does not match with current amount
                        if (!(await updateIngredient(menuItem.id, await findInventoryIdByName(newIngredient.inventoryItem.name), newIngredient.amount, tsql))) {
                            return false;
                        }
                    }
                    break;
            ...
            if (!found) {
                if (!(await deleteIngredient(menuItem.id, await findInventoryIdByName(currentIngredient.inventoryItem.name, tsql)))) {
                    return false;
                }
            }
        ...
        for (const newIngredient of menuItem.ingredients) { //if new ingredient is not in the current ingredient entries
            
            let found = false;

            for (const currentIngredient of currentIngredients) {
                if (newIngredient.inventoryItem.name == currentIngredient.inventoryItem.name) { //if name of ingredient matches with current ingredient
                    found = true;
                    break;
                }
            }
            ...
            if (!found) {
                if (!(await addIngredient(menuItem.id, await findInventoryIdByName(newIngredient.inventoryItem.name, tsql), newIngredient.amount), tsql)) { //if addingredient returns false
                    return false;
                }
            }
        }
...
export async function deleteMenuItem(menuItem: MenuItem , tsql = psql): Promise<boolean> {
    return transact<boolean, postgres.Error, any>(tsql, new Error("SQL Error in deleteMenuItem", undefined, {menuItem:menuItem}), async (isql, _) => {
        ...
        const deletedMenuItem = new MenuItem(itemId,menuItem.name,menuItem.type,menuItem.price,menuItem.netPrice,menuItem.popularity,menuItem.ingredients,menuItem.seasonal); //creates listing for deleted menu item
...
export async function addSeasonalItem(menuItem: MenuItem , tsql = psql): Promise<boolean> {
    return transact<boolean, postgres.Error, any>(tsql, new Error("SQL Error in addSeasonalItem", undefined, {menuItem:menuItem}), async (isql, _) => {
        ...
        if(menuItem.seasonal == null){ //ignores if seasonal is null
            return false;
        }
        const result = await isql`
...
export async function updateSeasonalItem(menuItem: MenuItem , tsql = psql): Promise<boolean> {
    return transact<boolean, postgres.Error, any>(tsql, new Error("SQL Error in updateSeasonalItem", undefined, {menuItem:menuItem}), async (isql, _) => {
        ...
        if(seasonalItemExists){ //if seasonalitem exists
            if( menuItem.seasonal != null){
                ...
                if(updateQuery.length > 0){ //if at least a parameter exists
                    return true;
                }
                return false;   
                ...
                if(deleteQuery.length > 0){ //if at least a parameter exists
                    return true;
                }
                return false;               
            }
        } else{
            if( menuItem.seasonal != null){ //add seasonal item entry
...
export async function addOrUpdate(menuItem: MenuItem , tsql = psql): Promise<boolean> {
    return transact<boolean, postgres.Error, any>(tsql, new Error("SQL Error in addOrUpdate", undefined, {menuItem:menuItem}), async (isql, _) => {
        ...
        if (itemExists) { //if item exists, update it
            const updateQuery = await isql `
        ...
        return false;   
        ...
        } else { //if item does not exist, insert a new one
            ...
            if(insertQuery.length > 0){ //if at least a parameter exists
                return true;
            }
            return false;  
        }
...
export async function existsInDatabase(name: string , tsql = psql): Promise<boolean> {
    return transact<boolean, postgres.Error, any>(tsql, new Error("SQL Error in existsInDatabase", undefined, {name:name}), async (isql, _) => {
        const result = await isql`
...
export async function removeIngredients(name: string, ingredientsToRemove: string[], tsql = psql): Promise<boolean> {
    return transact<boolean, postgres.Error, any>(tsql, new Error("SQL Error in removeIngredients", undefined, {name:name}), async (isql, _) => {
        ...
        if(!(await existsInDatabase(name, tsql))){ //checks if menu item exits
            return false;
        }
        ...
        for (const ingredientName of ingredientsToRemove) {
            ...
            if (!inventoryId) { //skip if no ingrident found
                continue;
            }
