...
export async function getInventoryItemByName(name: string, tsql = psql): Promise<InventoryItem | null> {
    return transact<InventoryItem | null, postgres.Error, any>(tsql, new Error("SQL error in getInventoryItemByName", undefined, name), async (isql, _) =>{
        const result = await isql`SELECT id, name, qty, avg_cost, min_qty, max_qty FROM inventory WHERE name = ${name}`;
        if (result.length > 0)
        {
            const item = result[0];
...
export async function addOrUpdateInventoryItem(item: InventoryItem, tsql = psql): Promise<boolean> {
    return transact<boolean, postgres.Error, any>(tsql, new Error("SQL error in getInventoryItemByName", undefined, item), async (isql, _) =>{
        ...
        if (result.length > 0)
        {
            const subResult = await isql`UPDATE inventory SET qty = ${item.quantity}, avg_cost = ${item.averageCost}, min_qty = ${item.minQuantity}, max_qty = ${item.maxQuantity} WHERE name = ${item.name}`
            return subResult.length > 0;
        }
        else 
        {
            const subResult = await isql`INSERT INTO inventory (name, qty, avg_cost, min_qty, max_qty) VALUES (${item.name}, ${item.quantity}, ${item.averageCost}, ${item.minQuantity}, ${item.maxQuantity})`;
            return subResult.length > 0;
        }
...
export async function removeIngredient(menuItemName: string, inventoryName: string, tsql = psql): Promise<boolean> {
    return transact<boolean, postgres.Error, any>(tsql, new Error("SQL error in getInventoryItemByName", undefined, {menuItemName, inventoryName}), async (isql, _) =>{
        ...
        const findInventoryIdSQL = await isql`SELECT id FROM inventory WHERE name = ${inventoryName}`;
        ...
        if(checkIngredientExistsStmt.length > 0 && checkIngredientExistsStmt[0].existsInInventory)
        {
            ...
            {
                const deleteIngredientStmt = await isql`DELETE FROM ingredients WHERE item_id = ${menuItemId} AND inventory_id = ${inventoryId} AND amount <= 1`;
            }
        }
        else
        {
...
export async function request(inventoryName: string, amount: number, tsql = psql): Promise<boolean> {
    return transact<boolean, postgres.Error, any>(tsql, new Error("SQL error in getInventoryItemByName", undefined, {inventoryName, amount}), async (isql, _) =>{
        ...
        const updateQtySQL = await isql`UPDATE inventory SET qty = qty + ${amount} WHERE name = ${inventoryName} AND qty + ${amount} <= max_qty`;
