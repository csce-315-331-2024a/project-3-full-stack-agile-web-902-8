...
const formatDataForGraph = (data: BarData[], numberOfColumns: number): BarData[] => {
...
const orderAndLimitData = (data: BarData[], limit: number, yAxisOrder: string[]): BarData[] => {
...
const DataPage = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const responseFrequentlySoldPairs = await fetch(`/api/getFrequentlySoldPair?begin=${formattedStartDate}&end=${formattedEndDate}`);
      if (!responseFrequentlySoldPairs.ok) {
        throw new Error(`Failed to fetch frequently sold pairs: ${responseFrequentlySoldPairs.statusText}`);
      }
      const dataFrequentlySoldPairs = await responseFrequentlySoldPairs.json();
      setFrequentlySoldPairs(dataFrequentlySoldPairs);
  
      // API request for aggregated inventory items for product usage
      const resInventory = await fetch(`/api/aggregateInventory?start=${startDate.getTime()}&end=${endDate.getTime()}`);
      if (!resInventory.ok) {
        throw new Error(`Failed to fetch aggregated inventory data: ${resInventory.statusText}`);
      }
      const inventoryJson = await resInventory.json();
      const orderedInventoryData = orderAndLimitData(inventoryJson, yAxisOrderProductUsage.length, yAxisOrderProductUsage);
      setInventoryData(orderedInventoryData);
  
      // API request for aggregated menu items for sales report
      const resMenuItems = await fetch(`/api/aggregateMenuItems?beginTime=${startDate.getTime()}&endTime=${endDate.getTime()}`);
      if (!resMenuItems.ok) {
        throw new Error(`Failed to fetch aggregated menu items: ${resMenuItems.statusText}`);
      }
      const menuItemsJson = await resMenuItems.json();
      const orderedMenuData = orderAndLimitData(menuItemsJson, yAxisOrderSales.length, yAxisOrderSales);
    ...
    <div style={pageContainerStyle}>
      ...
      {!isLoading && !error && (
        <>
          ...
          <div className="report-section">
            <ScrollableList items={frequentlySoldPairs} title="What Sells Together" />
          </div>
